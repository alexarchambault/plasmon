// Originally based on https://github.com/scalameta/metals/blob/0ad0bc184f82dbd178d01f76913ea6bdfa98db14/metals/src/main/scala/scala/meta/internal/parsing/TokenEditDistance.scala
...
sealed trait TokenEditDistance {
  ...
  /** Converts a range position in the original document to a range position in the revised
    * document.
    *
    * This method behaves differently from the other `toRevised` in a few ways:
    *   - it should only return `None` in the case when the sources don't tokenize. When the
    *     original token is removed in the revised document, we find instead the nearest token in
    *     the original document instead. This behaviour can be toggled off by providing value of
    *     false for `fallbackToNearest`.
    *
    * If `adjustWithinToken` is true and the original and revised range are both contained within a
    * single (matching) token, then adjust the returned range to match the start/end offsets of the
    * input range with respect to the token.
    */
  def toRevised(
    range: l.Range,
  ...
  final def toOriginal(
    revisedPos: l.Position
  ): Either[EmptyResult, Position] =
    toOriginal(revisedPos.getLine, revisedPos.getCharacter)
  ...
  /** Converts a bsp diagnostic in the original document to the revised document. `None` will be
    * returned if conversion of any of the diagnostic's underlying text edits cannot be revised.
    */
  final def toRevised(
...
object TokenEditDistance {
  ...
  private implicit class XtensionPositionRangeLSP(val pos: Position) extends AnyVal {
  ...
  def apply(
    ...
    if (!isScala && !isJava)
      ...
      val tokenizedRevised  = trees.tokenized(module, revisedInput)
      val tokenizedOriginal = trees.tokenized(module, originalInput)
      val result = (tokenizedRevised, tokenizedOriginal) match {
        case (Some(Tokenized.Success(revised)), Some(Tokenized.Success(original))) =>
          Right(
            TokenEditDistance.fromTokens(
              originalInput,
              original.tokens,
        ...
        case (Some(err: Tokenized.Error), _) =>
          Left(err.message)
        case (_, Some(err: Tokenized.Error)) =>
          Left(err.message)
  ...
  private class MatchingToken[A](val original: A, val revised: A) {
