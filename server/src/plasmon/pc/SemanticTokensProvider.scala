// Based on https://github.com/scalameta/metals/blob/0ad0bc184f82dbd178d01f76913ea6bdfa98db14/metals/src/main/scala/scala/meta/internal/metals/SemanticTokensProvider.scala
...
/** Provides semantic tokens of file according to the LSP specification.
  */
private object SemanticTokensProvider {

  private def getTokens(isScala3: Boolean, text: String): Tokens =
    if (isScala3) {
  ...
  /** Main method. Fist, Codes are convert to Scala.Meta.Tokens. And a semantic token, which is
    * composed by 5 Ints are provided for each meta-token. If a meta-token is Idenitifier, the
    * attributes (e.g. constant or not) are gotten using presentation Compler. All semantic tokens
    * is flattend to a list and returned.
    */
  def provide(
    nodes: List[Node],
    ...
    if (nodes.isEmpty) {
      ...
      val tokens = Try(getTokens(isScala3, params.text())) match {
        case Failure(t) =>
          scribe.error(s"Could not find semantic tokens for: ${params.uri()}", t)
          PlasmonHelpers.Tokens(Array.empty)
  ...
  private def getTypeAndMod(
    ...
    tk match {
      case ident: Token.Ident if isOperatorName(ident) =>
        (getTypeId(l.SemanticTokenTypes.Operator), 0, nodesIterator)
  ...
  /** This function returns -1 when capable Type is nothing. TokenTypes that can be on multilines
    * are handled in another func. See Token.Comment in this file.
    */
  private def typeModOfNonIdentToken(
    ...
    val tokenType: Int =
      tk match {
        ...
        case _: Token.ModifierKeyword => getTypeId(l.SemanticTokenTypes.Modifier)
        case _: Token.Keyword         => getTypeId(l.SemanticTokenTypes.Keyword)
        case _: Token.KwNull          => getTypeId(l.SemanticTokenTypes.Keyword)
        case _: Token.KwTrue          => getTypeId(l.SemanticTokenTypes.Keyword)
        case _: Token.KwFalse         => getTypeId(l.SemanticTokenTypes.Keyword)

        // extends Symbolic keywords
        case _: Token.Hash            => getTypeId(l.SemanticTokenTypes.Keyword)
        case _: Token.Viewbound       => getTypeId(l.SemanticTokenTypes.Operator)
        case _: Token.LeftArrow       => getTypeId(l.SemanticTokenTypes.Operator)
        case _: Token.Subtype         => getTypeId(l.SemanticTokenTypes.Keyword)
        case _: Token.RightArrow      => getTypeId(l.SemanticTokenTypes.Operator)
        case _: Token.Supertype       => getTypeId(l.SemanticTokenTypes.Keyword)
        case _: Token.At              => getTypeId(l.SemanticTokenTypes.Keyword)
        case _: Token.Underscore      => getTypeId(l.SemanticTokenTypes.Variable)
        case _: Token.TypeLambdaArrow => getTypeId(l.SemanticTokenTypes.Operator)
        case _: Token.ContextArrow    => getTypeId(l.SemanticTokenTypes.Operator)

        // Constant
        case _: Token.Constant.Int | _: Token.Constant.Long |
            _: Token.Constant.Float | _: Token.Constant.Double =>
          getTypeId(l.SemanticTokenTypes.Number)
        case _: Token.Constant.String | _: Token.Constant.Char =>
          getTypeId(l.SemanticTokenTypes.String)
        case _: Token.Constant.Symbol => getTypeId(l.SemanticTokenTypes.Property)

        // Comment
        case _: Token.Comment => getTypeId(l.SemanticTokenTypes.Comment)

        // Interpolation
        case _: Token.Interpolation.Id | _: Token.Interpolation.SpliceStart =>
          getTypeId(l.SemanticTokenTypes.Keyword)
        case _: Token.Interpolation.Start | _: Token.Interpolation.Part |
            _: Token.Interpolation.SpliceEnd | _: Token.Interpolation.End =>
          getTypeId(l.SemanticTokenTypes.String) // $ symbol
    ...
    val tokenModifier: Int =
      tokenType match {
        case _ if tokenType == getTypeId(l.SemanticTokenTypes.Variable) =>
          1 << getModifierId(l.SemanticTokenModifiers.Readonly)
        case _ => 0
      }
  ...
  private def trySoftKeyword(tk: Token): Integer =
    tk match {
      case SoftKeywordsUnapply.KwAs() => getTypeId(l.SemanticTokenTypes.Keyword)
      case SoftKeywordsUnapply.KwDerives() =>
        getTypeId(l.SemanticTokenTypes.Keyword)
      case SoftKeywordsUnapply.KwEnd() => getTypeId(l.SemanticTokenTypes.Keyword)
      ...
      case SoftKeywordsUnapply.KwOpaque() =>
        getTypeId(l.SemanticTokenTypes.Keyword)
      case SoftKeywordsUnapply.KwOpen() => getTypeId(l.SemanticTokenTypes.Keyword)
  ...
  private def isOperatorName(ident: Token.Ident): Boolean =
    ...
    def loop(
      ...
      parts match {
        ...
        case (part, partIdx) :: next =>
          ...
          val tokenMod =
            if (tokenType == getTypeId(l.SemanticTokenTypes.Variable))
              1 << getModifierId(l.SemanticTokenModifiers.Readonly)
            else 0
  ...
  private def getUsingTokenType(part: String, idx: Int): Int =
    idx match {
      case 0 =>
        if (part == "//>") getTypeId(l.SemanticTokenTypes.Comment) else -1
      case 1 =>
        if (part == "using") getTypeId(l.SemanticTokenTypes.Keyword) else -1
      case 2 => getTypeId(l.SemanticTokenTypes.Variable)
      case _ => getTypeId(l.SemanticTokenTypes.String)
    }
    ...
    docstringTokens.foreach { tk =>
      ...
      val (commentHighlight, commentDelta) =
        convertTokensToIntList(
          ...
          delta,
          getTypeId(l.SemanticTokenTypes.Comment)
        )
  ...
  private def makeStringTokens(
    ...
    def emitCurrent() = {
      ...
      if (current.nonEmpty) {
        emitToken(current, getTypeId(l.SemanticTokenTypes.String))
        currentPart.clear()
      }
    }
    ...
    def emitEscape(special: String) =
      emitToken(
        special,
        getTypeId(l.SemanticTokenTypes.Regexp)
      )
  ...
  private case class DocstringToken(
    start: Int,
    end: Int,
    text: String,
    tokenType: Integer,
      ...
      m.subgroups match {
        ...
        case "@throws" :: param :: _ if param != null =>
          ...
          val paramToken = DocstringToken(
            ...
            m.end(2),
            param,
            getTypeId(l.SemanticTokenTypes.Class),
            0
          )
        ...
        case keyword :: param :: _ if param != null && keyword != null =>
          ...
          val paramToken = DocstringToken(
            ...
            m.end(2),
            param,
            getTypeId(l.SemanticTokenTypes.Variable),
            1 << getModifierId(l.SemanticTokenModifiers.Readonly)
          )
          List(keywordToken, paramToken)
        // [[apiLink]]
  ...
  private object EscapableString {
