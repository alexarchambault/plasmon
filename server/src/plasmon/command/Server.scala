...
object Server extends caseapp.Command[ServerOptions] {
  ...
  def run(options: ServerOptions, remainingArgs: RemainingArgs): Unit = {
    ...
    System.err.println(s"PID: ${ProcessHandle.current().pid()}")
    ...
    def commandServer(pool: ExecutorService): CommandServer = new CommandServer {

      val entryPoint: CommandsEntryPoint = new CommandsEntryPoint {
        def progName = "plasmon"
        def commands: Seq[caseapp.Command[_]] =
          remoteCommands.map(_.instance(server, indexer, client, lspServer, pools))
