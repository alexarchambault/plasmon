...
// method initially generated by ChatGPT
async function openFromWorkspacePath(absPathOnDisk: string) {
  const wf = vscode.workspace.workspaceFolders?.[0]
  if (!wf) throw new Error("No workspace folder open")
  const rel = path.relative(wf.uri.fsPath, absPathOnDisk)
  const uri = vscode.Uri.joinPath(wf.uri, rel)
      ...
      client?.sendRequest(ExecuteCommandRequest.type, { command: "plasmon/debugSymbolIndex", arguments: [uri] }).then(
        async (resp: Resp) => {
          ...
          if (resp.error) {
            ...
            let path0 = vscode.Uri.parse(resp.contentUri).fsPath
            await openFromWorkspacePath(path0)
          }
        },
        (err) => {
